...
std::string GetEnvOrDefaultPath(const char* env_name, const char* lib_name)
{
	if (auto path = getenv(env_name))
		return path;
	...
	)
{
	options.add_options()
		...
		("ewasm", po::bool_switch(&enableEwasm), "tries to automatically find an ewasm vm and enable ewasm test-execution.")
...
bool CommonOptions::parse(int argc, char const* const* argv)
{
	...
	if (vmPaths.empty())
	{
		std::string evmone = GetEnvOrDefaultPath("ETH_EVMONE", evmoneFilename);
		if (!evmone.empty())
	...
	if (enableEwasm) {
		std::string hera = GetEnvOrDefaultPath("ETH_HERA", heraFilename);
		if (!hera.empty())
