...
bool EVMHost::checkVmPaths(vector<boost::filesystem::path> const& _vmPaths)
{
	...
	for (auto const& path: _vmPaths)
	{
		...
		{
			if (evmVmFound)
				throw std::runtime_error("Multiple evm1 evmc vms defined. Please only define one evm1 evmc vm.");
			evmVmFound = true;
		}
		...
		{
			if (ewasmVmFound)
				throw std::runtime_error("Multiple ewasm evmc vms where defined. Please only define one ewasm evmc vm.");
			ewasmVmFound = true;
		}
	}
	return evmVmFound;
